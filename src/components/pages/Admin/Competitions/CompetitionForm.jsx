...
import {
  CreateCompetition,
  UpdateCompetition
} from './Competition.mutations.graphql';
...
class CompetitionsForm extends Component {
  state = {
    error: null
  };
  ...
  render() {
    ...
    const query = isEdit ? UpdateCompetition : CreateCompetition;
        ...
        <Mutation
          ...
          update={(cache, { data: { createCompetition } }) => {
            ...
            data.allSports = update.addItem({
              list: data.allSports,
              item: { ...createCompetition, updatedAt: getCurrentDate() },
              sort: true
            });
...
CompetitionsForm.defaultProps = {
  item: {}
};

export default withToast(CompetitionsForm);
